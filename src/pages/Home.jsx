...
function Home() {
 ...
 // const { posts, setPosts } = usePosts();
  ...
  const handleLike = (index) => {
    // Créez une copie de l'état des posts et mettez à jour le nombre de likes
    const newPosts = [...posts];
    newPosts[index].likes = (newPosts[index].likes || 0) + 1; // Assurez-vous que 'likes' existe et incrémente
    setPosts(newPosts); // Mettez à jour l'état des posts
  };
  
  const handleCommentClick = (index) => {
    // Alterner la visibilité du champ de commentaire pour chaque post
  ...
  const handleCommentSubmit = (e, index) => {
    ...
    // Vérification si le tableau 'comments' existe pour ce post, sinon on l'initialise
    if (!newPosts[index].comments) {
      newPosts[index].comments = [];
    }
  
    // Ajout du commentaire à l'index spécifié
    newPosts[index].comments.push(newComment);
    
    // Mise à jour de l'état des posts avec le commentaire ajouté
    setPosts(newPosts);
  
    // Réinitialisation du champ de commentaire
    setNewComment('');
  };
